...
def fit_invariant_mass(hist, mass_range=(1.72, 2.04), label=""):
    ...
    mass = ROOT.RooRealVar("mass", "#it{M} (KK#pi)", mass_range[0], mass_range[1], "GeV/c^{2}")
    
    # Import the histogram into a RooDataHist
    data_hist = ROOT.RooDataHist("data_hist", "Data Histogram", ROOT.RooArgList(mass), hist)
    ...
    background = ROOT.RooExponential("background", "Background Exponential", mass, slope)
    ...
    signal_integral = signal.createIntegral(ROOT.RooArgSet(mass), ROOT.RooFit.NormSet(ROOT.RooArgSet(mass)),
                                            ROOT.RooFit.Range("signal_region")).getVal()
    background_integral = background.createIntegral(ROOT.RooArgSet(mass), ROOT.RooFit.NormSet(ROOT.RooArgSet(mass)),
                                                    ROOT.RooFit.Range("signal_region")).getVal()
    ...
    signal_yield_err = signal_yield * sig_frac_err / sig_frac_val if sig_frac_val != 0 else 0
    background_yield_err = background_yield * sig_frac_err / (1 - sig_frac_val) if sig_frac_val != 1 else 0
    ...
    if background_yield > 0:
        ...
        sb_ratio_err = sb_ratio * ((signal_yield_err / signal_yield)**2 +
                                   (background_yield_err / background_yield)**2)**0.5
        significance_err = significance * ((signal_yield_err / signal_yield)**2 +
                                            ((signal_yield_err + background_yield_err) /
                                             (signal_yield + background_yield))**2)**0.5
    else:
    ...
    latex.DrawLatex(0.15, 0.84, f"#mu=({mean_val:.3f}#pm{mean_err:.3f}) GeV/#it{{c}}^{{2}}")
    latex.DrawLatex(0.15, 0.80, f"#sigma=({sigma_val:.3f}#pm{sigma_err:.3f}) GeV/#it{{c}}^{{2}}")
    latex.DrawLatex(0.60, 0.84, f"#it{{S}}={signal_yield:.2f}#pm{signal_yield_err:.2f}")
    latex.DrawLatex(0.60, 0.80, f"#it{{B}}={background_yield:.2f}#pm{background_yield_err:.2f}")
    ...
    canvas.SaveAs(f"{outdir}/fit_result_{hist.GetName()}.pdf")
...
def set_style(histo, ytitle):
    ...
    histo.GetYaxis().SetRangeUser(1.e-6, 2.02)
...
# Main script
infile = "/home/spolitan/alice/analyses/hf-mc/postprocess/inputs/AnalysisResults_data_occupancy_020.root"
indir = "hf-task-flow-charm-hadrons"
thn_name = "hSparseFlowCharm"
outdir = "data_occupancy_020"
...
outfile = ROOT.TFile(f"{outdir}/projected_thn_dataocc_020.root", "RECREATE")
...
for icent, (cent_min, cent_max) in enumerate(
    ...
    for iocc, (occ_min, occ_max) in enumerate(
        ...
        for ipt, (pt_min, pt_max) in enumerate(
            ...
            dict = fit_invariant_mass(hmass, mass_range=(1.72, 2.04),
    ...
    for iocc, (occ_min, occ_max) in enumerate(
        ...
        hist_ratio_s[-1].SetName(f"hist_ratio_s_cent{cent_min}_{cent_max}_occ{occ_min}_{occ_max}")
    ...
    for iocc, (occ_min, occ_max) in enumerate(
        ...
        hist_ratio_b[-1].SetName(f"hist_ratio_b_cent{cent_min}_{cent_max}_occ{occ_min}_{occ_max}")
    ...
    for iocc, (occ_min, occ_max) in enumerate(
        ...
        hist_ratio_soverb[-1].SetName(f"hist_ratio_soverb_cent{cent_min}_{cent_max}_occ{occ_min}_{occ_max}")
        hist_ratio_soverb[-1].Divide(hist_soverb[icent][iocc], hist_soverb[icent][0], 1.0, 1.0, "B")
    ...
    for iocc, (occ_min, occ_max) in enumerate(
        ...
        hist_ratio_signif[-1].SetName(f"hist_ratio_signif_cent{cent_min}_{cent_max}_occ{occ_min}_{occ_max}")
        hist_ratio_signif[-1].Divide(hist_signif[icent][iocc], hist_signif[icent][0], 1.0, 1.0, "B")
    ...
    for iocc, (occ_min, occ_max) in enumerate(
        ...
        hist_ratio_mean[-1].SetName(f"hist_ratio_mean_cent{cent_min}_{cent_max}_occ{occ_min}_{occ_max}")
        hist_ratio_mean[-1].Divide(hist_mean[icent][iocc], hist_mean[icent][0], 1.0, 1.0, "B")
    ...
    for iocc, (occ_min, occ_max) in enumerate(
        ...
        hist_ratio_sigma[-1].SetName(f"hist_ratio_sigma_cent{cent_min}_{cent_max}_occ{occ_min}_{occ_max}")
        hist_ratio_sigma[-1].Divide(hist_sigma[icent][iocc], hist_sigma[icent][0], 1.0, 1.0, "B")
    ...
    canvas_cent_ratio_s.SaveAs(f"{outdir}/cratio_b_cent{cent_min}_{cent_max}.pdf")
    canvas_cent_ratio_b.SaveAs(f"{outdir}/cratio_s_cent{cent_min}_{cent_max}.pdf")
    canvas_cent_ratio_soverb.SaveAs(f"{outdir}/cratio_soverb_cent{cent_min}_{cent_max}.pdf")
    canvas_cent_ratio_signif.SaveAs(f"{outdir}/cratio_signif_cent{cent_min}_{cent_max}.pdf")
    canvas_cent_ratio_mean.SaveAs(f"{outdir}/cratio_mean_cent{cent_min}_{cent_max}.pdf")
    canvas_cent_ratio_sigma.SaveAs(f"{outdir}/cratio_sigma_cent{cent_min}_{cent_max}.pdf")
